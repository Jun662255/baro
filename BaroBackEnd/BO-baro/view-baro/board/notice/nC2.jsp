...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#frm_write").submit(function(e) {
			...
			ajax({
				...
				data : {
					modl_seq : $("#modl_seq option:selected").val()
				},
	...
	function changeTypeCalendar(obj) {
		...
		$("#dsply_st_dttm").daterangepicker(obj,
					...
					$(data.rows)
							.each(
									function(index) {
										...
										if (sitListCnt == 1) {
											html += "<option  class='passOption' value="+ this.SYS_CD +" selected='selected' >"
													+ this.SYS_NM + "</option>"
											getModlInf(this.SYS_CD);
											return false;
										} else {
											html += "<option class='passOption' value="+ this.SYS_CD +">"
													+ this.SYS_NM + "</option>"
											$("#sys_info").closest('div')
													.show();
										}
									});
			...
			success : function(data) {
				if (data.RESULT == _boolean_success) {
					...
					if (boardCtgInfoList.length > 0) {
						...
						$("#ctegry_dtl_cd")
								.append(
										"<option value='' ctegry_mst_cd=''>없음</option>");
					}
				}
			}
		});
	...
	function validChk() {
		...
		if ($("#dsply_yn").prop("checked")
				&& (isEmpty($("#dsply_st_dttm").val()) || isEmpty($(
						"#dsply_ed_dttm").val()))) {
	...
	function regDocInf(doc_seq, cts) {
		...
		var ctegry_mst_cd = $("#ctegry_dtl_cd option:selected").attr(
				"ctegry_mst_cd");
		...
		formData.append("notc_yn", $(":input:radio[name=notc_yn]:checked")
				.val());
		formData.append("otp_stat", "PUBLIC");
		formData.append("titl", $("#titl").val());
		formData.append("cts", cts);
		formData.append("cmt_pms_stat", $('input[name="cmt_pms_stat"]:checked')
				.val());
		formData.append("fl_seqs", params);
		formData.append("modl_tp", $("#modl_seq option:selected").attr(
				"modl_tp"));
		...
		formData.append("ctegry_dtl_cd", $("#ctegry_dtl_cd option:selected")
				.val());
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (_boolean_success == json.RESULT) {
					...
					var panelId = parent.$(
							"a[href=#" + window.frameElement.id + "]").closest(
							"li").remove().attr("aria-controls");
	...
	function resetValue() {
		...
		$('input[name="cmt_pms_stat"]:radio[value="ALLOW"]').prop('checked',
				true);
	...
	<div class="frameWrap">
		<div class="subCon">
			...
			<table class="tblType1">
				...
				<tr>
					<th>
						<label for="titl" class="vv">제목</label>
					</th>
						<td colspan="5">
							<span class="txt_pw">
								<input type="text" name="titl" id="titl" class="w100" data-byte="250" title="제목" />
							</span>
						</td>
				</tr>
				<tr>
					<td colspan="6" class="editorTd">
						...
						<div class="board_writeFiles">
							...
							<div class="info">
								<p class="text-small">
									첨부파일 <span id="fileCount"><em>0</em>개 (0 MB / 21.00 MB)</span>
								</p>
								<div>
							...
							<div class="files-prog" style="display: none;">
								<p id="file-message"></p>
								<div class="progress">
									<div class="progress-bar" id="progBar" role="progressbar"
